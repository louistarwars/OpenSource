...
:root {
  ...
  --net: #0b7285; --ok: #2b8a3e; --code-bg: #f1f1ee;
}
@media (prefers-color-scheme: dark) {
  :root {
    ...
    --net: #66d9e8; --ok: #69db7c; --code-bg: #26262a;
  }
}
...
.facts { display: grid; grid-template-columns: repeat(auto-fit, minmax(170px, 1fr)); gap: 10px; margin-top: 14px; }
...
<main>
  ...
  <div class="controls">
    ...
    <label><input type="checkbox" id="onlydev"> deviations only</label>
    <input type="search" id="q" placeholder="filter (task, file, text)">
  </div>
  <div class="scroll"><table id="grid"></table></div>
  <footer>Generated by detangle __VERSION__. Rows highlighted in orange are points where the scheduler deviated from asyncio's default FIFO order.</footer>
</main>
...
(function () {
  ...
  const facts = [
    [data.kind, 'failure kind'], [data.deviations, 'deviations from asyncio order'],
  ...
  function render() {
    ...
    for (const e of trace.events) {
      ...
      if (onlyDev && !e.deviation.length) continue;
      ...
      const cls = e.deviation.length ? 'dev' : (e.verb === 'raise' || e.verb === 'error') ? 'bad' : '';
      let cells = '';
      for (const a of cols) {
